...
from .utils import timestamp_to_hex, to_hex, xai_address_to_evm, xai_hash_to_evm
...
def translate_block(xai_block: dict, include_txs: bool = False) -> dict:
    ...
    evm_block = {
        ...
        # Roots (merkle/state)
        "transactionsRoot": xai_hash_to_evm(xai_block.get("merkle_root", "")),
    ...
    if include_txs:
        ...
        evm_block["transactions"] = [
            translate_transaction(tx, height, idx) for idx, tx in enumerate(xai_txs)
        ]
    else:
        # Just return transaction hashes
        evm_block["transactions"] = [
            xai_hash_to_evm(tx.get("txid", tx) if isinstance(tx, dict) else tx) for tx in xai_txs
        ]
